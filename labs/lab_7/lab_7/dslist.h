...
public:
    ...
    declares dslist() to have properties in variables called head_ tail_ and size_
                                                     */
    dslist(const dslist<T>& old) { this->copy_list(old); } // more declaration
...
void dslist<T>::destroy_list() {
    Node<T>* tmp;
    while (size_ != 0){
        tmp = head_->next_; // head is first element of the dslist
        delete head_;
        head_ = tmp;
        --size_;
    }
