...
int pathfinder(int x,int y,vector<vector<bool> >& b,vector<Point>& pv){
    vector<Point> dummy;
    if (b[x][y])
        return 0;
    int paths=0;
    bool added=false;
    if (x>0){
        paths+=pathfinder(x-1,y,b,pv);
        if (paths>0 && added==false){
            added=true;
            Point p(x,y);
            pv.push_back(p);
        }
    ...
    if (y>0){
        if (added)
            paths+=pathfinder(x,y-1,b,dummy);
        else
            paths+=pathfinder(x,y-1,b,pv);
        if (paths>0 && added==false){
            added=true;
            Point p(x,y);
            pv.push_back(p);
...
int main(int argc, char* argv[]) {
  ...
  // Start here with your code...
    vector<Point> pv;
    cout << pathfinder(start_x,start_y,blocked_grid,pv)<<endl;
    for (unsigned int i=pv.size(); i-->0; ){
